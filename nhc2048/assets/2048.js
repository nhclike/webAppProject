...
function setWidthHeight(){
	...
	//按照约定的:cell宽高是整体宽高的4/21
	cell_width = width*(4/21);
	cell_height = height*(4/21);

	base_margin_left = (1/4)*cell_width;
	base_margin_top = (1/4)*cell_height;
	$('#gridPanel').css({
		width:width,
	    height:height
	});
...
var game={
	...
	init:function(){//初始化所有格子div的html代码
		...
		//gap("gridPanel").style.width=this.CN*116+16+"px";
		//设置id为gridPanel的高为: RN*116+16+"px";
		//gap("gridPanel").style.height=this.RN*116+16+"px";
		var grids=[];
		var cells=[];
		//r从0开始,到<RN结束，每次++
		...
		gap("gridPanel").innerHTML=grids.join("")
			                    +cells.join("");
		setWidthHeight();
	},
	start:function(){//开始游戏方法
		...
		gap("top").innerHTML=this.getTop();
		this.state=this.RUNNING;//初始化游戏状态为运行
		gap("gameOver").style.display="none";
	...
	updateView:function(){
		...
		gap("score").innerHTML=this.score;
		//span对象
		...
		//	找到id为finalScore的span，设置内容为游戏的分数
			gap("finalScore").innerHTML=this.score;
		//	找到id为gameOver的div，显示出来
			gap("gameOver").style.display="block";
